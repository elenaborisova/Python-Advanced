...
class Game:
    ...
    def tick(self):
        if self.points_a >= 10 or self.points_b >= 10:
            self.game_over = True
            return
        self.__perform_border_check()
        self.__perform_paddle_hit_check()
        x, y = self.__ball_pos
        self.__ball_pos = (x + self.__ball_delta_x, y + self.__ball_delta_y)

    def __perform_paddle_hit_check(self):
        x, y = self.__ball_pos
    ...
    def __perform_border_check(self):
        x, y = self.__ball_pos
